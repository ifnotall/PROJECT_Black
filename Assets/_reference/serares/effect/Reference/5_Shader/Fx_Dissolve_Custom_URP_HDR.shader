Shader "Archipin/FX/Fx_Dissolve_Custom_URP_HDR"
{
    Properties
    {
        ...
        Pass
        {
           ...
           #include "Packages/com.unity.render-pipelines.universal/ShaderLibrary/Lighting.hlsl"
            ...
            struct VertexOutput
            {
               ...
               float2 customData1 : TEXCOORD5;
               float2 customData2 : TEXCOORD6;
...

            {
                ...
                o.uv3 = v.uv.xy * _DissolveTex_ST.xy + _DissolveTex_ST.zw;
            ...
            {          
              ...
              float2 NoiseUV = tex2D(_NoiseTex, float2(i.uv1.x - (_Time.x*15) * _NoiseXSpeed, i.uv1.y - _Time.y * _NoiseYSpeed)) * _NoisePower;  
              float2 MainUV = (float2(i.uv.x - (_Time.x*15) * _MainTexXSpeed, i.uv.y - _Time.y * _MainTexYSpeed)) + NoiseUV;
              float4 c = tex2D(_MainTex, MainUV - float2(customData1.x, customData1.y)) * _TintColor * _AddInt;
                     c.rgb = MixFog(c.rgb, i.fogCoord);
              float4 alpha = tex2D(_AlphaTex, i.uv2);

              clip(tex2D (_DissolveTex, i.uv3).rgb - (_DissolveAmount + customData2.x));
