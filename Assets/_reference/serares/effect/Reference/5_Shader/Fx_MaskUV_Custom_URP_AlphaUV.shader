Shader "Archipin/FX/Fx_MaskUV_Custom_URP_AlphaUV"
{
    Properties
    {
        ...
        _AlphaXSpeed("Alpha Tex X Speed", Range(-100,100)) = 0
		_AlphaYSpeed("Alpha Tex Y Speed", Range(-10,10)) = 0
        [Space(20)]
        ...
        Pass
        {
           ...
           #include "Packages/com.unity.render-pipelines.universal/ShaderLibrary/Lighting.hlsl"
            ...
            struct VertexOutput
            {
               ...
               float2 customData1 : TEXCOORD4;
               float2 customData2 : TEXCOORD5;
            ...
            {          
              ...
              float2 customData1 = i.customData1;
              float2 customData2 = i.customData2;
              
              float4 s = tex2D(_SecondTex, i.uv3);
              float2 NoiseUV = float2(i.uv1.x - (_Time.x*15) * _NoiseXSpeed, i.uv1.y - _Time.y * _NoiseYSpeed);  
              float2 NoiseFinal = tex2D(_NoiseTex, NoiseUV - float2(customData2.x, customData2.y)) * _NoisePower;
              float2 MainUV = (float2(i.uv.x - (_Time.x*15) * _MainTexXSpeed, i.uv.y - _Time.y * _MainTexYSpeed)) + NoiseFinal;
              float4 c = tex2D(_MainTex, MainUV - float2(customData1.x, customData1.y)) * _TintColor * _AddInt * s;
                     c.rgb = MixFog(c.rgb, i.fogCoord);
              float4 alpha = tex2D(_AlphaTex, float2(i.uv2.x - (_Time.x*15) * _AlphaXSpeed, i.uv2.y - _Time.y * _AlphaYSpeed));
